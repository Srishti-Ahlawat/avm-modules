...
resource "azurerm_virtual_desktop_workspace_application_group_association" "workappgr" {
  ...
  # application_group_id = lookup(data.azurerm_virtual_desktop_application_group.existing_app_groups, each.value["app_group_key"], null)["id"]
  # workspace_id         = module.avm-res-desktopvirtualization-workspace[each.key].resource.id
  depends_on = [module.avm-res-desktopvirtualization-workspace]
}
