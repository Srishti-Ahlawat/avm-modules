...
module "avm-res-compute-virtualmachine" {
  ...
  location                                               = var.location
  name                                                   = var.name != null ? var.name : format("${local.virtual_machine_name}-%03s", (count.index + 1))
  ...
  os_disk = {
    ...
    name                      = var.operating_system_disk_name != null ? var.operating_system_disk_name : format("osdisk-${local.virtual_machine_name}-%03s", (count.index + 1))
  }
